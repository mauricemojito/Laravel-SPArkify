            ...
            <div class="col-md-3 spark-settings-tabs">
                <aside>
                    ...
                    <ul class="nav flex-column mb-4 ">
                        <li class="nav-item " v-if="ownsTeam">
                  ...
                  18c2.7737 0 5.2177-1.4116 6.6528-3.5556C14.6268 13.517 12.3738 13 10 13s-4.627.517-6.6528 1.4444z "></path>
                        ...
                        <li class="nav-item " v-if="spark.createsAdditionalTeams">
                            <router-link class="nav-link" :to="spark.routePrefix+'/settings#/channels'">
                                <svg class="icon-20 " viewBox="0 0 20 20 " xmlns="http://www.w3.org/2000/svg ">
              ...
              0-1.2-.2.8-1 1.3-2.4 1.3-3.8s0-2.7-1-3.8l1-.2z " />
                ...
                <aside v-if="ownsTeam && spark.canBillTeams">
                    <h3 class="nav-heading ">
                        {{ $t('Team Billing') }}
                    </h3>
                    <ul class="nav flex-column mb-4 ">
                        <li class="nav-item " v-if="spark.hasPaidTeamPlans">
...
<script>
    ...
    export default {
        ...
        props: ["user"],
        mixins: [base],
        components: {TeamProfile, TeamMembership, Subscription, PaymentMethod, Invoices},
        data() {
            return {
                ownsTeam: true
            }
        ...
        beforeRouteEnter :function (to, from, next) {
            axios.get('/api/guards/onteam/'+ to.params.id ).then(response => {
                axios.get('/api/guards/ownsteam/'+ to.params.id ).then(response => {
                    next(vm => {
                        vm.ownsTeam = response.data.ownsTeam
                    });
                });
            }).catch(error => {
                next('/404')
...
<i18n>
{
    ...
    "ru": {
        ...
        "Membership": "Состав команды",
        "View All Teams": "Просмотреть все команды",
        "Your Settings": "Ваши настройки",
        "Team Billing": "Командный счет",
